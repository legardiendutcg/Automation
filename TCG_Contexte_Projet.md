...
> Dernière mise à jour : **Session #13 — 31/05/2026** · **Versions actuelles : Sales 1.14.0 / Master 1.14.0 (roll-back de vente, #27) / Integrator 1.2.1**
>
...
- **Au début de la prochaine session** :
  ...
  - 🧪 **À TESTER EN NAVIGATEUR — livraisons Session #13 (RIEN n'a encore été testé)** · *déployer Sales 1.14.0 + Master 1.14.0 ENSEMBLE, Ctrl+Shift+R* :
    ...
    - **E. Reliquats #12 (jamais testés)** — (18) **dédup txId** : dupliquer une ligne `ventes_detail` (même txId+clé) → `DUP_SKIPPED` + anomalie `DUPLICATE_TXID` ; (19) **acheteur imposé** : event rempli → champ verrouillé pour tous, vide → champ libre obligatoire ; (20) **Integrator 1.2.1** : un import test écrit bien dans `cartes_stock_et_transactions` ; (21) **verrou 2 appareils** : Master sur 2 appareils → lease/TTL, « Forcer la reprise », anti-ping-pong.
...
> ✅ **`tcg-converter.html` corrigé en #12** — ⚠️ et **non en #9** comme l'indiquait par erreur ce document : le renommage (priorité #9.1) avait été **documenté comme livré mais jamais appliqué au fichier**, qui restait sur `Cartes_Stock_&_Transactions` → l'Integrator était **cassé** (400 « Unable to parse range »). Réparé en **1.2.0** : `SHEET_NAME` → `cartes_stock_et_transactions` **et** helper `a1()` porté à **toutes** les plages (cartes + onglets LOG/BACKUP), comme Sales/Master.
...
> ⚠️ **Toutes les livraisons #13 passent `node --check` mais AUCUNE n'a été testée en navigateur.** Checklist de test complète en **§0** (à faire avant prod).
...
- **[Integrator 1.2.0 — RÉPARATION 🔴]** L'Integrator était **cassé** : `SHEET_NAME` était resté sur `Cartes_Stock_&_Transactions` (le renommage #9.1 documenté n'avait jamais été appliqué au fichier) → 400 « Unable to parse range » sur toutes ses opérations. Corrigé → `cartes_stock_et_transactions`, **+ helper `a1()`** porté à toutes les plages (cartes + LOG/BACKUP ; références d'onglet aussi quotées dans les formules `COUNTIF`/`SUMIF`).
- **[Integrator 1.2.1 — config + CSP]** `SAFETY_GSHEET_ID` **renseigné** (`1c-HD5z…DtI`, feuille dédiée logs/backups) → journal + backups d'injection opérationnels. **CSP** ajoutée.
- **[CSP sur les 3 apps — #23]** Balise `<meta Content-Security-Policy>` en tête de `<head>` (Sales **1.10.2** / Master **1.11.1** / Integrator **1.2.1**). `default-src 'self'` ; script/style inline autorisés (apps mono-fichier) ; `connect-src` limité à Sheets + OAuth Google (+ les 5 proxies de scraping côté Integrator seulement) ; `object-src 'none'` ; `base-uri 'self'`. Protège surtout contre l'**exfiltration** réseau et l'injection de **script externe**. `frame-ancestors` ignoré en `<meta>` (nécessiterait un en-tête HTTP, indispo sur GitHub Pages).

### 🧪 Reste à tester en navigateur (rien testé en #12) — voir la liste détaillée en §0
**La CSP est le seul vrai point sensible** (peut casser l'OAuth si un endpoint manque) : la console affichera tout blocage CSP, retrait = supprimer la balise. Tester aussi : dédup doublon `txId`, acheteur imposé (event rempli/vide), import Integrator, et le reliquat **jamais testé** du #11 : scénario **2 appareils** (verrou + reprise).
...
- **Versioning** : #18 a pris **Master 1.11.0** (minor, fix fonctionnel significatif, Master seul) → l'**auth GIS glisse en 1.12.0** (jalon coordonné). Patches CSP : Sales 1.10.2 / Master 1.11.1 / Integrator 1.2.1.
...
| 17 | ~~**[Integrator] Renommage onglet cartes + `a1()`**~~ ✅ | **Fait en #12 (1.2.0)** — ⚠️ et **non en #9** comme indiqué auparavant (jamais appliqué → l'Integrator était cassé). `SHEET_NAME` → `cartes_stock_et_transactions` **et** `a1()` porté à toutes les plages. |
